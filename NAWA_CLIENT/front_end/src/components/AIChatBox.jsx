  ...
  // Admin
  {
    ...
    patterns: [
      ...
      { q: /remove teacher|edit teacher|teacher management|manage teachers|edit teacher details/i, a: async () => {
        setPendingRedirectUrl('/admin/remove-teacher');
  ...
  // Teacher
  {
    ...
    patterns: [
      ...
      { q: /student|class|my students|my class|student list|show me the student list|list students|all students/i, a: async () => {
        ...
        return `<a href='/teacher-student-list' target='_blank' style='color:#0a66c2;text-decoration:underline;'>Click here to view the student list</a>`;
      } },
...
const AIChatBox = () => {
  ...
  const handleQA = async (text, showAll = false) => {
    ...
    if (role !== 'teacher' && /notice|show me notices|notices/i.test(lower)) {      let url = getApiUrl('/api/notices');
      if (role === 'admin') url = getApiUrl('/api/notices/admin');
      else if (role === 'student') url = getApiUrl('/api/notices/student');
  ...
  function ChatBubble({ message }) {
    ...
    if (message.role === 'user') {
      return (
        <div className="flex justify-end mb-2">
          <div className={`px-4 py-2 rounded-2xl max-w-[70%] shadow text-sm text-white ${message.userRole === 'admin' ? 'bg-blue-700' : message.userRole === 'teacher' ? 'bg-green-600' : message.userRole === 'student' ? 'bg-orange-500' : 'bg-gray-500'} modern-bubble-user`}>{messageContent}</div>
    ...
    return (
      <div className="flex items-end gap-2 mb-2">
        ...
        <div className="modern-bubble-bot bg-white text-gray-900 px-4 py-2 rounded-2xl rounded-bl-sm max-w-[70%] shadow text-sm whitespace-pre-line border border-blue-100" dangerouslySetInnerHTML={{ __html: messageContent }} />
      </div>
    );
  }
