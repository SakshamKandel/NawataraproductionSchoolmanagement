...
import { getFieldError } from "../utils/formUtils.js";
...
const LoginForm = () => {
  ...
  const formBackendFunc = async (data) => {
    try {
      ...
      showSuccessToast(response.data.alertMsg || "Logged in successfully!");
      ...
      if (error.response) {
        // Safely handle error response data - check for message properties
          ...
          <form className="space-y-6" onSubmit={handleSubmit(formBackendFunc)}>
            <div>
              ...
              <div className="mt-1.5">
                ...
                />
                {errors.email && (
                  <p className="mt-1.5 text-sm text-red-600 font-medium">{getFieldError(errors, 'email')}</p>
                )}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="mt-1.5 relative">
                ...
                {errors.password && (
                  <p className="mt-1.5 text-sm text-red-600 font-medium">{getFieldError(errors, 'password')}</p>
                )}
              </div>
            </div>
