...
const YearEndManagement = () => {
  ...
  const fetchAcademicYearStatus = async () => {
    try {
      const response = await axios.get(getApiUrl('/api/year-end/academic-year'), { withCredentials: true });
  ...
  const fetchPromotionStatus = async (class_name) => {
    ...
    try {
      setLoading(true);
      const response = await axios.get(getApiUrl(`/api/year-end/promotion-status/${class_name}`), { withCredentials: true });
  ...
  const downloadAllClassData = async (classNum = null, isBatch = false) => {
    ...
    try {
      ...
      const allStudentsResponse = await fetch(getApiUrl('/api/year-end/export-all-students'), {
        method: 'GET',
        credentials: 'include'
      });
      ...
      if (isBatch) {
        ...
        for (let i = 1; i <= 6; i++) {
          const classResponse = await fetch(getApiUrl(`/api/year-end/export-class-for-promotion?classNum=${i}`), {
            method: 'GET',
            credentials: 'include'
          });
        ...
        const classResponse = await fetch(getApiUrl(`/api/year-end/export-class-for-promotion?classNum=${classNum}`), {
          method: 'GET',
          credentials: 'include'
        });
        ...
        if (classResponse.ok) {
          ...
          if (parseInt(classNum) === 5) {
            const class6Response = await fetch(getApiUrl('/api/year-end/export-class-for-promotion?classNum=6'), {
              method: 'GET',
              credentials: 'include'
            });
  ...
  const handlePromoteStudentsConfirmed = async (classNum) => {
    try {
      ...
      const response = await axios.post(getApiUrl('/api/year-end/promote-students'), {
        class_name: classNum,
    ...
    try {
      ...
      for (let classNum = 6; classNum >= 1; classNum--) {
        try {
          ...
          const promoteRes = await axios.post(getApiUrl('/api/year-end/promote-students'), {
            class_name: classNum,
            academicYear: academicYear.year,
            password
          }, { withCredentials: true });
  ...
  const handleClearPayroll = async () => {
    try {
      ...
      setLoading(true);
      await axios.post(getApiUrl('/api/year-end/clear-payroll'), {
        academicYear: academicYear.year,
        password
      }, { withCredentials: true });
  ...
  const handleDownloadStudents = async () => {
    try {
      ...
      const response = await fetch(getApiUrl('/api/year-end/export-all-students'), {
        method: 'GET',
        credentials: 'include'
      });
