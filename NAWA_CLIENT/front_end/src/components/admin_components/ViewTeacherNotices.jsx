...
const ViewTeacherNotices = () => {
  ...
  const handleApprove = async (id) => {
    try {
      const response = await axios.patch(
        getApiUrl(`/teacher-notices/${id}/approve`),
        {},
        { withCredentials: true }
      );
      ...
      toast.error(err.response?.data?.message || 'Failed to approve notice');
    }
  };
  ...
  const handleDeny = async (id) => {
    try {
      const response = await axios.patch(
        getApiUrl(`/teacher-notices/${id}/deny`),
        {},
        { withCredentials: true }
      );
      ...
      console.error('Error denying notice:', err);
      toast.error(err.response?.data?.message || 'Failed to deny notice');
    }
  };
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {currentNotices.map((notice) => (
                      <tr key={notice.id} className="hover:bg-blue-50 transition-colors duration-150">
          ...
          <div className="bg-white rounded-lg max-w-2xl w-full p-6 shadow-xl border border-gray-200 transform transition-all duration-200 ease-in-out animate-scale-in">
            ...
            {selectedNotice.status === 'pending' && (
              <div className="flex gap-2 mt-4">
                <button
                  onClick={() => handleApprove(selectedNotice.id)}
                  className="bg-green-500 text-white px-4 py-2 rounded"
                >
                  Approve
                </button>
                <button
                  onClick={() => handleDeny(selectedNotice.id)}
