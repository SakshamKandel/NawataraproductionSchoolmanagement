import sequelize from './config/database.js';

async function runFeeMigration() {
  try {
    console.log('Starting fee structure migration...');
    
    // Test database connection
    await sequelize.authenticate();
    console.log('Database connection established.');
    
    // Check current table structure
    const [columns] = await sequelize.query("SHOW COLUMNS FROM ClassFees");
    const columnNames = columns.map(col => col.Field);
    console.log('Current ClassFees columns:', columnNames);
    
    let migrationNeeded = false;
    
    // Add new columns if they don't exist
    if (!columnNames.includes('transportationFee')) {
      console.log('Adding transportationFee column...');
      await sequelize.query("ALTER TABLE ClassFees ADD COLUMN transportationFee FLOAT NOT NULL DEFAULT 0");
      migrationNeeded = true;
    } else {
      console.log('transportationFee column already exists.');
    }
    
    if (!columnNames.includes('examFee')) {
      console.log('Adding examFee column...');
      await sequelize.query("ALTER TABLE ClassFees ADD COLUMN examFee FLOAT NOT NULL DEFAULT 0");
      migrationNeeded = true;
    } else {
      console.log('examFee column already exists.');
    }
    
    // Handle computerFee migration
    if (columnNames.includes('computerFee')) {
      console.log('Migrating computerFee data to transportationFee...');
      // Copy data from computerFee to transportationFee if transportationFee is 0
      await sequelize.query(
        "UPDATE ClassFees SET transportationFee = computerFee WHERE computerFee IS NOT NULL AND transportationFee = 0"
      );
      
      console.log('Removing computerFee column...');
      await sequelize.query("ALTER TABLE ClassFees DROP COLUMN computerFee");
      migrationNeeded = true;
    } else {
      console.log('computerFee column already migrated.');
    }
    
    // Remove admissionFee if it exists
    if (columnNames.includes('admissionFee')) {
      console.log('Removing admissionFee column...');
      await sequelize.query("ALTER TABLE ClassFees DROP COLUMN admissionFee");
      migrationNeeded = true;
    } else {
      console.log('admissionFee column already removed.');
    }
    
    if (migrationNeeded) {
      console.log('Migration completed successfully!');
    } else {
      console.log('No migration needed - database is already up to date.');
    }
    
    // Verify final table structure
    const [finalColumns] = await sequelize.query("SHOW COLUMNS FROM ClassFees");
    console.log('\nFinal ClassFees table structure:');
    finalColumns.forEach(col => {
      console.log(`  ${col.Field}: ${col.Type} ${col.Null === 'NO' ? 'NOT NULL' : 'NULL'} ${col.Default !== null ? `DEFAULT ${col.Default}` : ''}`);
    });
    
    // Check if there are any existing records
    const [existingRecords] = await sequelize.query("SELECT COUNT(*) as count FROM ClassFees");
    console.log(`\nFound ${existingRecords[0].count} existing fee structure records.`);
    
    if (existingRecords[0].count === 0) {
      console.log('Creating default fee structures...');
      
      const defaultFeeStructures = [
        { className: 'Nursery', monthlyFee: 2000, transportationFee: 500, examFee: 300 },
        { className: 'L.K.G.', monthlyFee: 2200, transportationFee: 500, examFee: 300 },
        { className: 'U.K.G.', monthlyFee: 2400, transportationFee: 500, examFee: 300 },
        { className: 'One', monthlyFee: 2600, transportationFee: 600, examFee: 400 },
        { className: 'Two', monthlyFee: 2800, transportationFee: 600, examFee: 400 },
        { className: 'Three', monthlyFee: 3000, transportationFee: 600, examFee: 400 },
        { className: 'Four', monthlyFee: 3200, transportationFee: 700, examFee: 500 },
        { className: 'Five', monthlyFee: 3400, transportationFee: 700, examFee: 500 },
        { className: 'Six', monthlyFee: 3600, transportationFee: 700, examFee: 500 }
      ];
      
      for (const feeStructure of defaultFeeStructures) {
        await sequelize.query(
          "INSERT INTO ClassFees (className, monthlyFee, transportationFee, examFee, createdAt, updatedAt) VALUES (?, ?, ?, ?, NOW(), NOW())",
          {
            replacements: [feeStructure.className, feeStructure.monthlyFee, feeStructure.transportationFee, feeStructure.examFee]
          }
        );
      }
      
      console.log('Default fee structures created successfully.');
    }
    
    console.log('\n✅ Fee structure migration completed successfully!');
    console.log('You can now use the fee management system.');
    
  } catch (error) {
    console.error('❌ Migration failed:', error);
    console.error('Error details:', error.message);
  } finally {
    await sequelize.close();
    console.log('Database connection closed.');
    process.exit(0);
  }
}

// Run the migration
runFeeMigration();